  ...
  <div class="container">
    ...
    <div class="input-area">
      <div class="role-row">
        ...
        <input v-model="aiRole" class="role-input" type="text" placeholder="回答者の立場（例：教育の専門家・UXデザイナー）">
      </div>
...
// 立場プレフィックス（任意）
const roleParts = [];
if (userRole.value && userRole.value.trim()) roleParts.push(`質問者の立場: ${userRole.value.trim()}`);
if (aiRole.value && aiRole.value.trim()) roleParts.push(`回答者の立場: ${aiRole.value.trim()}`);
const rolePrefix = roleParts.length ? roleParts.join('\n') + '\n\n' : '';

if (executionMode === 'reverse') {
  prompt = `${intro}「${inputText.value}」というキーワードに対して、常識や前提を逆転させた視点から、思考を促す問いを1つ新たに作成してください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というキーワードについて、他の分野や用途に応用・拡張することで、思考を促す問いを1つ新たに作成してください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というキーワードに対して、キーワード内の単語をひとつを取り出して連想される単語を1つ掛け合わせて、思考を促す問いを1つ新たに作成してください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というテーマについて、立場や視点を切り替えて考えることで、思考を促す問いを1つ作成してください。たとえば「子どもだったら？」「外国人だったら？」「動物だったら？」など、視点の転換を活かしてください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というテーマについて、未来の技術や社会、価値観の変化を想定しながら、思考を促す問いを1つ作成してください。10年後、50年後など、時間軸をずらして考えてください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というテーマについて、制限や条件を設けることで、思考を促す問いを1つ作成してください。たとえば「予算が○○円しかない」「時間が○○分しかない」「道具が使えない」など、制約を活かしてください。前の問いはリセットし、内容を引き継がないようにしてください。
...
例:（80文字以内の答えを3つ、番号付きで）`;
  ...
  prompt = `${intro}「${inputText.value}」というテーマについて、特定の感情（不安・喜び・怒り・期待など）を通して考えることで、思考を促す問いを1つ作成してください。前の問いはリセットし、内容を引き継がないようにしてください。
