...
if (executionMode === 'reverse') {
  ...
  prompt = `${rolePrefix}${intro}「${inputText.value}」というテーマについて、様々な制限や条件を設けることで、思考を促す問いを1つ作成してください。たとえば「予算が○○円しかない」「時間が○○分しかない」「道具が使えない」など、制約を活かしてください。前の問いはリセットし、内容を引き継がないようにしてください。\n\n出力形式は以下のようにしてください：\n問い:（50文字以内で）（制約には「」を付ける）\n例:（80文字以内の答えを3つ、番号付きで）`;
}
